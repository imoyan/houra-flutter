...
and `SPEC-036` Matrix event/messages vectors. It is not published, canonical,
...
`SPEC-031`, `SPEC-032`, `SPEC-033`, `SPEC-034`, `SPEC-035`, and `SPEC-036`.
...
`SPEC-035` / `SPEC-036` JSON envelopes, but it does not own HTTP, retries,
